function fill_files_data(data){
    ...
    $.each(data.response.docs, function(key, value) {
        ...
        if (html_safe_id.endsWith(".html")){
            ...
            if (div){
                div.innerHTML = "<button type=\"button\" rel=\"downloadbutton\" title=\"Download\" class=\"btn btn-success btn-simple btn-link\" onclick=\"localStorage.setItem('peaco_report', 'https://labcas.jpl.nasa.gov/nist/data-access-api/download?id="+html_safe_id+"'); window.location.href = '" + ui_path("/a/peaco.html") + "'\">"+
...
function fill_datasets_children(data){
    ...
    $.each(data.response.docs, function(key, value) {
        ...
        if (value.contains_image){
            ...
            image_div = "<button id='view_"+id_safe_id+"' type=\"button\" rel=\"tooltip\" title=\"View\" onclick=\"Cookies.set('login_redirect', '" + ui_path("/d/index.html?dataset_id=" + html_safe_id) + "'); window.location.replace('" + ui_path("/fqc/index.html?version=5.1.0") + "');\" class=\"btn btn-simple btn-link\" style='position: absolute;left: -50px; top: 50%; transform: translateY(-50%); color: red'>"+
                "<i class=\"fa fa-image\"></i>"+
            "</button>";
        }
    ...
    if ( dataset_html != ""){
        ...
        $( "#dataset_stat_template" ).load(ui_path("/templates.html #dataset_stat_template"));
        setTimeout(function() {
            $("#collection_datasets_len").html(dataset_count);
        }, 2000);
    }else{
        $( "#dataset_stat_template" ).load(ui_path("/templates.html?version=5.1.0 #dataset_name_template"));
    }
...
function fill_dataset_details_data(data){
    ...
    if (obj.id == get_var["dataset_id"]){
        ...
        $("#collection_name").html("<a href=\"" + ui_path("/c/index.html?collection_id=" + collectionid) + "\">"+collectionid+"</a>");
