...
function fill_collections_data(data){
          ...
          $("#collection-table tbody").append(
            "<tr>"+
                ...
                "<td class=\"td-actions\">"+
                    ...
                    "</td>"+
            "</tr>");
        console.log("Debug change4.4");
    });
    console.log("Debug change6");
    $('#loading').hide(500);
    $table.bootstrapTable({
            toolbar: ".toolbar",
            clickToSelect: true,
    ...
    console.log("Debug change7");
    //activate the tooltips after the data table is initialized
    $('[rel="tooltip"]').tooltip();

    console.log("Debug change8");
    $(window).resize(function() {
        $table.bootstrapTable('resetView');
    });
    console.log("Debug change9");
}
...
function fill_datasets_data(data){
    ...
    $.each(data.response.docs, function(key, value) {
        ...
        if (value.id.split(/\//)[1] == get_var["collection_id"]){
            console.log("collection_level");
            query_labcas_api(localStorage.getItem('environment')+"/data-access-api/files/select?q=DatasetId:"+value.id+"&wt=json&sort=FileName%20asc&indent=true&rows=10000", fill_collection_level_files);
            ...
            query_labcas_api(localStorage.getItem('environment')+"/data-access-api/files/select?q=DatasetId:"+value.id+"&wt=json&sort=FileName%20asc&indent=true&rows=10000", fill_collection_metadata);
