...
function infer_ui_base_path() {
    var path = location.pathname || "";
    if (path === "/nist" || path.indexOf("/nist/") === 0) {
        return "/nist";
    }
    if (path === "/labcas-ui" || path.indexOf("/labcas-ui/") === 0) {
        return "/labcas-ui";
    }
    return "";
}

function get_ui_base_path() {
    var stored = normalize_ui_base_path(localStorage.getItem("ui_base_path"));
    if (stored) {
        return stored;
    }
    return infer_ui_base_path();
}

function ui_path(path) {
    var base = get_ui_base_path();
    if (!path) {
        return base || "";
    }
    if (path[0] !== "/") {
...
$().ready(function() {
	...
	setTimeout(function(){
        var ui_base_path = get_ui_base_path();
        var ui_root = ui_base_path || "";
        if (!(location.pathname === ui_root + "/index.html" || location.pathname === ui_root + "/" || location.pathname === ui_root || location.pathname === ui_root + "/o/index.html")){
...
function redirect_to_login(){
    if (localStorage.getItem("allow_redirect") == "true"){
        window.location.replace(ui_path("/index.html"));
    }else{
...
function login_redirect(){
        if (Cookies.get("login_redirect")){
            ...
            window.location.replace(ui_path("/s/index.html?search=*"));
        }
}
...
function initCookies(){
	if(!Cookies.get("token") || Cookies.get("token") == "None"){
		$.ajax({
			url: ui_path('/assets/conf/environment.cfg?26'),
			dataType: 'json',
			async: false,
...
function resume_download(){
	...
	window.location.replace(ui_path("/download.html?version=5.1.0"));
}
...
function selected_files(formname){
    ...
    if (get_var["dataset_id"] && get_var["dataset_id"] != "undefined"){
	    Cookies.set("login_redirect", ui_path("/d/index.html?dataset_id="+get_var["dataset_id"]))
    }else if (get_var["search"]){
	    Cookies.set("login_redirect", ui_path("/s/index.html?search="+get_var["search"].replace("&","%26")))
    }
...
function download_files(formname){
    ...
    if (filesize > parseInt(localStorage.getItem("file_size_threshold_for_download_alert_trigger"))){
        ...
        $('#redirectModal .modal-footer').html("<button type='submit' onclick=\"window.location.replace('" + ui_path("/download.html?version=5.1.0") + "')\" class='btn btn-info btn-fill pull-right' data-dismiss='modal'>Continue</button><div class=\"col-md-6\" style=\"padding-right:0px text-align: right\"><button type=\"button\" class=\"btn btn-danger btn-simple \" style=\"float: right\" data-dismiss=\"modal\">Cancel</button></div>");
        $('#redirectModal_continue').hide();
        $('#redirectModal').modal('show');
...
function download_dataset(dataset){
    ...
    if (dataset == "hierarchy_query" && localStorage.getItem("hierarchy_file_query")){
        var file_query = localStorage.getItem("hierarchy_file_query");
        url = localStorage.getItem('environment')+"/data-access-api/files/select?q=*"+file_query+"&wt=json&rows=5000";
    }else{
        dataset = dataset.replace("%5C%20","%20").replace("%20","%5C%20").replace(" ","%5C%20");
        url = localStorage.getItem('environment')+"/data-access-api/files/select?q=DatasetId:"+dataset+"&wt=json&rows=5000";
    }
...
function download_collection_wizard(){
    ...
    url = localStorage.getItem('environment')+"/data-access-api/files/select?q=CollectionId:"+collection+"&wt=json&rows=5000";
...
function reset_search_filters(){
	...
	window.history.replaceState({}, document.title, ui_path("/s/index.html?search=*"));
}
function set_search_filters(params){
	...
	if (params["search"] && params["search"] != "*" && params["search"] != ""){
		...
		window.location.replace(ui_path("/s/index.html?search="+params["search"].replace("&","%26")));
	}else{
		window.location.replace(ui_path("/s/index.html?search=*"));
	}
}
...
function generate_image_file_list(data){
	...
	if (check_dicom_multi()){
	       window.location.replace(ui_path("/i/mindex.html?version=5.1.0"));
	}else{
	       window.location.replace(ui_path("/i/index.html?version=5.1.0"));
	}
    }else if(image_type === "omeros"){
	window.location.replace(ui_path("/o/index.html?version=5.1.0"));
    }else{
       window.location.replace(ui_path("/z/index.html?version=5.1.0"));
    }
}
...
function submitImage(formname, dataset){
    if ( $('#check_all:checked').length || dataset){
        ...
        if (dataset){
            ...
            if (dataset == "hierarchy_query" && localStorage.getItem("hierarchy_file_query")){
                var file_query = localStorage.getItem("hierarchy_file_query");
                url = localStorage.getItem('environment')+"/data-access-api/files/select?q=*"+file_query+"&wt=json&indent=true&rows=5000";
            }else{
                dataset = dataset.replace("%5C%20","%20").replace("%20","%5C%20").replace(" ","%5C%20");
                url = localStorage.getItem('environment')+"/data-access-api/files/select?q=DatasetId:"+dataset+"&wt=json&indent=true&rows=5000";
            }
        ...
        var get_var = get_url_vars();
            ...
            if (get_var["dataset_id"] && get_var["dataset_id"] != "undefined"){
                    ...
                    Cookies.set("login_redirect", ui_path("/s/index.html?search="+get_var["search"].replace("&","%26")))
                        ...
                        Cookies.set("login_redirect", ui_path("/f/index.html?file_id="+get_var["file_id"]))
            }
            if (image_type == "dicoms"){
		if (check_dicom_multi()){
	       window.location.replace(ui_path("/i/mindex.html?version=5.1.0"));
...
function check_omero_image(image_dataset, image_name, version, show_flag, fileid, show_flag){
	...
	$.each(omero_datasets, function(key, val){
		...
		var details_cmd = "window.open(\"" + ui_path("/f/index.html?file_id=" + fileid) + "\");";
...
function pdf_viewer(file_id){
    ...
    window.location.replace(ui_path("/pdf/"));
}
